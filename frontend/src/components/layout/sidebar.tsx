...
export function Sidebar({ userRole }: { userRole: string }) {
    ...
    const filtered = navigation.filter(item => {
        if (userRole === "Teacher" && ["الطلاب", "المعلمون", "أدوات المدير", "المالية"].includes(item.name)) return false
        return true
    })
    ...
    return (
        <aside style={{ width: 240, flexShrink: 0 }} className="flex flex-col h-full bg-white border-l border-gray-200">
            ...
            <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-0.5">
                <p className="px-3 pb-2 text-[10px] font-bold uppercase tracking-widest" style={{ color: 'var(--text-subtle)' }}>
                    التنقل
                </p>
                {filtered.map((item) => {
